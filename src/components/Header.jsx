    ...
    <header className="mx-auto w-full max-w-6xl px-4 pt-5 sm:px-6">
      <nav className="rounded-xl border border-zinc-800 bg-zinc-900/70 px-4 py-4 backdrop-blur">
        <div className="flex items-center justify-between sm:justify-center">
          <button
            type="button"
            className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-zinc-700 bg-zinc-800 text-zinc-200 transition hover:bg-zinc-700 sm:hidden"
        ...
        <ul className="mt-4 hidden items-center gap-7 sm:flex sm:justify-center">
          {navItems.map((item) => (
            <li key={item.to}>
              <Link
                className="text-base tracking-wide text-zinc-300 transition-colors hover:text-sky-400"
                to={item.to}
              >
                {item.label}
              </Link>
      ...
      >
        ...
        <ul className="flex flex-col gap-2">
          {navItems.map((item) => (
            <li key={item.to}>
              <Link
                className="block rounded-md px-3 py-2 text-zinc-200 transition hover:bg-zinc-800 hover:text-sky-400"
                to={item.to}
                onClick={() => setIsMenuOpen(false)}
              >
                {item.label}
